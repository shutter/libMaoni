...
static void readVertices(PlyFile* file, const int nVertices, Mesh &mesh) {
...
static void readTriangles(PlyFile* file, const int nFaces, Mesh &mesh) {
...
MESH_LOADER(ply, Stanford Triangle Format)
{
	...
	for (int i = 0; i < nPlyElems; ++i) {
		...
		if (equal_strings(elemNames[i], "vertex")) {
			...
			readVertices(file, nElems, mesh);
			BOOST_ASSERT( mesh.vertices.size() == static_cast< size_t >( nElems ) );
		} else if (equal_strings(elemNames[i], "face"))
			try {
				readTriangles(file, nElems, mesh);
				BOOST_ASSERT( mesh.triangles.size() == static_cast< size_t >( nElems ) );
				result = true;
